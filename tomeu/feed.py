...
class Feed:
    ...
    def _get_etag(self, c):
        c.execute("SELECT etag FROM cache WHERE feed_url=?", (self.url,))
        etag = c.fetchone()
        return etag

    def _get_hash(self, c):
        c.execute("SELECT hash from cache where feed_url=?", (self.url,))
        _hash = c.fetchone()
    ...
    def _needs_update(self, c):
        ...
        if etag and "etag" in self.d:
            ...
            c.execute(
                "UPDATE cache SET etag=? WHERE feed_url=?", (self.d.etag, self.url)
            )
            return True
        else:
            hash_ = self._generate_hash(self.d.entries)
            ...
            c.execute("UPDATE cache SET hash=? WHERE feed_url=?", (hash_, self.url))
            return True

    def _delete_old_entries(self, c):
        c.execute("SELECT last_delete FROM cache WHERE feed_url=?", (self.url,))
